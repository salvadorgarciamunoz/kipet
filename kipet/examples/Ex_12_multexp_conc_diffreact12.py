...
if __name__ == "__main__":
    ...
    opt_model = builder.create_pyomo_model(0.0,30.0) 
    ...
    opt_model1 = builder1.create_pyomo_model(0.0,30.0) 
    ...
    # defines the discrete points wanted in the concentration profile
    sim1.apply_discretization('dae.collocation', nfe=80, ncp=3, scheme='LAGRANGE-RADAU')
    ...
    builder.add_parameter('k5',init=0.2,bounds=(0.015,1))
    ...
    builder1.add_parameter('k5',init=0.2,bounds=(0.015,0.4))
    ...
    end_time = {'Exp1':30.0, 'Exp2':30.0}    
    datasets = {'Exp1': D_frame1, 'Exp2': D_frame2}
    sigmas = {'A':1e-10,'B':1e-10,'C':1e-11, 'D':1e-11,'E':1e-11,'F':1e-11,'G':1e-11,'H':1e-11,'device':0.0199}
    ...
    if with_plots:
        for k,v in results_pest.items():
            ...
            results_pest[k].C['G'].plot.line(legend=True, linestyle="-.")
            results_pest[k].C['H'].plot.line(legend=True, linestyle=":")
            plt.xlabel("time (s)")
            plt.ylabel("Concentration (mol/L)")
            plt.title("Concentration Profile")
            plt.show()

            results_pest[k].Z['A'].plot.line(legend=True)
            results_pest[k].Z['B'].plot.line(legend=True, linestyle="--")
            results_pest[k].Z['C'].plot.line(legend=True, linestyle="-.")
            results_pest[k].Z['D'].plot.line(legend=True, linestyle=":")
            results_pest[k].Z['E'].plot.line(legend=True, linestyle="-")
            results_pest[k].Z['F'].plot.line(legend=True, linestyle="--")
            results_pest[k].Z['G'].plot.line(legend=True, linestyle="-.")
            results_pest[k].Z['H'].plot.line(legend=True, linestyle=":")
            plt.xlabel("time (s)")
            plt.ylabel("Concentration (mol/L)")
            plt.title("Concentration Profile")
