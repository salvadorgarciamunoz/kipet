...
if __name__ == "__main__":
    ...
    builder.add_parameter('k1', init=1.0, bounds=(0.0,30.0)) 
    #There is also the option of providing initial values: Just add init=... as additional argument as above.
    builder.add_parameter('k2',init = 0.226, bounds=(0.0,70))
    ...
    #builder.add_spectral_data(datasets)
    ...
    sigmas = {'A':1e-9,'B':1e-9,'C':1e-9,'device':1e-6}
    
    options = dict()
    options['linear_solver'] = 'ma57'
    m, results_pyomo = run_param_est_for_mult_exp('ipopt',
                                      tee=True,
                                      solver_opts = options,
                                      variances=sigmas,
                                      datasets = datasets, 
    ...
    '''
    v_estimator = VarianceEstimator(opt_model, datasets)
