...
if __name__ == "__main__":
    ...
    D_frame2 = add_noise_to_signal(D_frame1, 0.00005)
    ...
    builder1.add_parameter('k1', init=1.0, bounds=(0.00,20)) 
    #There is also the option of providing initial values: Just add init=... as additional argument as above.
    builder1.add_parameter('k2',init = 0.224, bounds=(0.0,20))
    ...
    #There is also the option of providing initial values: Just add init=... as additional argument as above.
    builder2.add_parameter('k2',init = 0.2, bounds=(0.,20))
    #We have now added an extra local parameter here
    builder2.add_parameter('k3', init = 0.0001, bounds = (0.0, 1))
    ...
    def rule_odes(m,t):
        exprs = dict()
        exprs['A'] = -m.P['k1']*m.Z[t,'A']
        exprs['B'] = m.P['k1']*m.Z[t,'A']-m.P['k2']*m.Z[t,'B']-m.P['k3']*m.Z[t,'B']**3
        exprs['C'] = m.P['k2']*m.Z[t,'B']
        exprs['D'] = m.P['k3']*m.Z[t,'B']**3
        return exprs
    ...
    # add in the different models explicitly    
    ...
    sigmas = {'A':8.103255065335157e-11,'B':1.7990434594596e-10,'C':1.396502001183107e-10,'device':3.213282078821085e-06}
    sigmas1 = {'B': 1.6972405958549638e-10, 'A': 1.6721757632858386e-10, 'C': 1.616956123191492e-10, 'D': 1.6726259601491233e-10, 'device': 1.768162022069523e-06}
    variances = {'Exp1':sigmas, 'Exp2':sigmas1}
                 #, 'Exp3':sigmas}
    options1 = dict()
    
    #there are many potential solver options. Some of them are here
    
    #options1['linear_solver'] = 'ma77'
    #options1['ma57_pivot_order']=4
    #options1['ma57_automatic_scaling']='yes'
    #options1['bound_push']=1e-6
    #options1['mu_init']=1e-6
       
    options1['linear_solver'] = 'ma57'
    
    #options1['mu_init']=1e-4
    options1['mu_strategy']='adaptive'
    #options1['ma27_ignore_singularity'] = 'yes'
    options1['bound_push']=1e-6
    #options1['ma77_order']='metis'

    #options1['linear_solver'] = 'ma97'
    #options1['ma97_order']='best''metis'n
    
    #options1['ma86_order']='metis'
