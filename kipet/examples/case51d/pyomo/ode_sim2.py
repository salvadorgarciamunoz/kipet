...
def vel_rxns(m,t):
    ...
    step = 1.0/(1.0+exp(-m_sa/1e-6))
    rd = 0.0 #m.P['kd']*(C_sat-C_sa)**1.9*step
    r.append(rd)
    return r

def ensure_positive(m,t):
    return m.P['Csa'] >= m.Z[t,'SA']
model.pp = Constraint(model.time,rule=ensure_positive)

def mass_balances(m,t):
    r = vel_rxns(m,t)
    exprs = dict()
    V = m.X[t,'V']
    exprs['SA'] = -V*(r[0]+r[5]) - m.dXdt[t,'V']*m.Z[t,'SA']
    exprs['AA'] = -V*(r[0]+r[1]+r[3]) - m.dXdt[t,'V']*m.Z[t,'AA']
    exprs['ASA'] = V*(r[0]-r[1]+r[2]-r[4]) - m.dXdt[t,'V']*m.Z[t,'ASA']
...
def rule_Masa(m,t):
    PM = 180.157
    r = vel_rxns(m,t)
    return m.dXdt[t,'Masa'] == PM*m.X[t,'V']*r[4]
...
def rule_Msa(m,t):
    PM = 138.121
    r = vel_rxns(m,t)
    return m.dXdt[t,'Msa'] == -PM*m.X[t,'V']*r[5]
