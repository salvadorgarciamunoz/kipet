...
if __name__ == "__main__":
    ...
    filename =  os.path.join(dataDirectory,'Dij_tiv_G.txt')
    ...
    filename2 =  os.path.join(dataDirectory,"S_True_for_unwanted_G.csv")
    S_True = pd.read_csv(filename2,index_col = 0)

    # In this example, we know the magnitude of unwanted contribution.
    # Therefore, we can calculated the matched S according to "" to compare the results.
    ...
    S_matched = S_True + Sg
    S_matched.columns = ["Matched A","Matched B","Matched C"]
    ...
    if with_plots:
        ...
        ax = results_pyomo.S.plot.line(legend=True)
        S_True.plot(ax=ax)
        ...
        ax = results_pyomo.S.plot.line(legend=True)
        S_matched.plot(ax=ax, linestyle='dashed')
        plt.xlabel("Wavelength (cm)")
        plt.ylabel("Absorbance (L/(mol cm))")
        plt.title("Matched absorbance  Profile")
