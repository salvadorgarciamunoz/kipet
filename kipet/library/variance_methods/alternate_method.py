...
def solve_max_device_variance(var_est_object, solver, **kwds):
    ...
    if hasattr(var_est_object.model, 'non_absorbing'):
        ...
        list_components = [k for k in var_est_object._mixture_components if k not in var_est_object._non_absorbing]
        var_est_object._sublist_components = list_components
    
    if hasattr(var_est_object.model, 'known_absorbance'):
        warnings.warn("Overriden by species with known absorbance")
        list_components = [k for k in var_est_object._mixture_components if k not in var_est_object._known_absorbance]
        var_est_object._sublist_components = list_components
    ...
    if hasattr(var_est_object, '_abs_components'):
        var_est_object.component_set = var_est_object._abs_components
    else:
        var_est_object.component_set = var_est_object._sublist_components
    
    for t in var_est_object._meas_times:
        for l in set_A:
            D_bar = sum(var_est_object.model.Z[t, k] * var_est_object.model.S[l, k] for k in var_est_object.component_set)
    ...
    for t in var_est_object._meas_times:
        for l in set_A:
            D_bar = sum(value(var_est_object.model.Z[t, k]) * value(var_est_object.model.S[l, k]) for k in var_est_object.component_set)
...
# def solve_sigma_given_delta(var_est_object, solver, **kwds):
...
#     residuals, sigma_vals, stop_it, results = _solve_sigma_given_delta(var_est_object, solver, subset_lambdas= set_A, solver_opts = solver_opts, tee=tee, delta = delta)
