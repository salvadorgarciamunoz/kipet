...
class MultipleExperimentsEstimator(PEMixins, object):
    ...
    def __init__(self, reaction_models):
        ...
        for key in self.reaction_models.keys():
    ...
    def solve_consolidated_model(self, 
            ...
            def rule_objective(m):
                ...
                if hasattr(m, 'D'):
                    spectral_term = absorption_objective(m, 
                                                 ...
                                                 g_option=self.initialization_model[exp].G_contribution,
        ...
        def param_linking_rule(m, exp, param):
            ...
            if key == 0:
                ...
                if param in global_params and prev_exp != None:
                    return combined_model.experiment[exp].P[param] == combined_model.experiment[prev_exp].P[param]
                else:
                    return Constraint.Skip
        
        new_list_params_across_blocks=list()
        list_fixed_params=set()
        
        for exp in self.experiments:
            for param in combined_model.experiment[exp].P.keys():
                if combined_model.experiment[exp].P[param].is_fixed():
                    list_fixed_params.add(param)
        ...
        combined_model.parameter_linking = Constraint(self.experiments, new_list_params_across_blocks, rule = param_linking_rule)
            ...
            if key == 0:
                ...
                if wave in list_waves_across_blocks and prev_exp != None:
                    if comp in combined_model.experiment[prev_exp].mixture_components and comp in combined_model.experiment[exp].mixture_components:
                        return combined_model.experiment[exp].S[wave,comp] == combined_model.experiment[prev_exp].S[wave,comp]
                    else:
                        return Constraint.Skip
                else:
                    return Constraint.Skip
        ...
        if solver in ['k_aug', 'ipopt_sens']:
            ...
            optimizer.solve(combined_model, options=solver_opts,tee=tee)            
