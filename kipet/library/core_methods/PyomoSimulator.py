...
#         self._custom_data_given = hasattr(self.model, 'UD')
...
#             raise NotImplementedError("Fixing state variables is not allowed. Only algebraics can be fixed")
...
#         vars_to_init = {'X': [self._complementary_states, self._n_components],
#                         'Z': [self._mixture_components, self._n_complementary_states],
#                        }
        
#         for var, comp_set in vars_to_init.items():
        
#             if not hasattr(self, var):    
#                 continue
                
#             print(var, comp_set[0])
#             var_init = []
#             for t in self._alltimes:
#                 for k in comp_set[0]:
#                     if abs(self.model.init_conditions[k].value) > tol:
#                         var_init.append(self.model.init_conditions[k].value)
#                     else:
#                         var_init.append(1.0)
    
#             var_array = np.array(var_init).reshape((self._n_alltimes, comp_set[1]))
#             var_init_panel = pd.DataFrame(data=var_array,
#                                         columns=comp_set[0],
#                                         index=self._alltimes)
    
#             self.initialize_from_trajectory(var, var_init_panel)
...
#             print(f'the component is: {var.dim()}')
                 
            
#             if var.dim() > 1:# and variable_name != 'F0':
#                 try:
...
#             #         # Are these warnings really needed?
...
#             #             print(f'inner set\n{inner_set}')
#             #             print(f'single_traj: {single_trajectory}')
...
#                     print(f'Herer iss: {var}: {inner_set}')
...
#                         single_trajectory = trajectories[component]
#                         print(f'sing_traj: {single_trajectory}')
#                         values = interpolate_trajectory(inner_set, single_trajectory)
#                         for i, t in enumerate(inner_set):
#                             if not np.isnan(values[i]):
#                                 var[t, component].value = values[i]
#                 except:
#                     print(f'{variable_name} failed to initialize for some reason.')
#             else:
#                 print(f'{variable_name} has 1 dim')
...
#         if hasattr(self.model, 'UD'):
#             UD_var = self.model.UD
...
#         """All the results stuff starts here"""
