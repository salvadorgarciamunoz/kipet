...
class ParameterEstimator(Optimizer):
    ...
    def _define_reduce_hess_order(self):
        ...
        if not self._spectra_given:
            ...
            # added due to new structure for non_abs species, non-absorbing species not included in S and Cs as subset of C (CS):
            if hasattr(self, '_abs_components') and self.model_variance:  # added for removing non absorbing ones from first term in obj
                self.set_up_reduced_hessian(self._meas_times, self._abs_components, 'Cs', count_vars)
                count_vars += 1
            elif self.model_variance:
                self.set_up_reduced_hessian(self._meas_times, self._sublist_components, 'C', count_vars)
                count_vars += 1
            else:
                pass
    ...
    def _order_k_aug_hessian(self, unordered_hessian, var_loc):
        ...
        i = 0
        for vi in self._idx_to_variable.values():
            j = 0
            for vj in self._idx_to_variable.values():
                if n_vars ==1:
    ...
    def run_opt(self, solver, **kwds):
        ...
        # Options for unwanted contribution cannot be True at the same time. KH.L
        if self.unwanted_G and self.time_variant_G:
        ...
        elif self.unwanted_G and self.time_invariant_G:
        ...
        elif self.time_variant_G and self.time_invariant_G:
        ...
        # For time-invariant G, build omega matrix from St and Z_in,
        # and check rko to see whether or not G can be decomposed. KH.L
        ...
        # print(self.unwanted_G)
        # print(self.time_variant_G)
