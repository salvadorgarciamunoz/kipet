...
class TemplateBuilder(object):
    ...
    Attributes:
        ...
        _feed_time (set, optional): container of feed times
    ...
    def __init__(self, **kwargs):
       ...
       # self._feedadded = False #added for feeding points 0s
    ...
    def add_spectral_data(self, data):
        ...
        #print(len(self._meas_times))
        #rowsD=len(data.index)+len(self._feed_times)
        #D=np.zeros((rowsD,len(data.columns)))
        
        #for t in self._meas_times:#data.index)):
            #if t not in self._feed_times:#added
                ...
                #for l in range(len(data.columns)):
                    #D[t,l]=0
                    #self._feedadded = True
                #########
        #if self._is_D_deriv == True:
            #print("Warning! Since D-matrix contains negative values Kipet is assuming a derivative of D has been inputted")
        #elif self._feedadded ==True:
            #print("Warning! Since feeding points were added, D-matrix contains 0 values and Kipet is assuming a derivative of D has been inputted")
    def add_concentration_data(self, data):
        ...
        """
        if isinstance(data, pd.DataFrame):
            #still have to be changed for feeds!!!!
    ...
    def add_feed_times(self, times):
        ...
        for t in times:
            ...
            self._meas_times.add(t)#added here to avoid double addition
    ...
    def create_pyomo_model(self,start_time, end_time):#, modeljump=None):
        ...
        """
        #pyomo_model.pprint(filename="test.txt")
        # if modeljump is not None:
        #     print('#########################here##########################')
        #     #sys.exit()
        #     pyomo_model=modeljump.clone()
        #     pyomo_model.del_component(pyomo_model.Z_index)
        #     pyomo_model.del_component(pyomo_model.dZdt_index)
        #     pyomo_model.del_component(pyomo_model.C_index)
        #     pyomo_model.del_component(pyomo_model.X_index)
        #     pyomo_model.del_component(pyomo_model.dXdt_index)
        #     pyomo_model.del_component(pyomo_model.Y_index)
        #     pyomo_model.del_component(pyomo_model.S_index)
        #     pyomo_model.del_component(pyomo_model.D_index)
        #     pyomo_model.del_component(pyomo_model.odes_index)
        #     pyomo_model.del_component(pyomo_model.algebraic_consts_index_1)
        #     pyomo_model.del_component(pyomo_model.algebraic_consts_index)
        #     pyomo_model.del_component(pyomo_model.dZdt_disc_eq)
        #     pyomo_model.del_component(pyomo_model.dZdt_disc_eq_index)
        #     pyomo_model.del_component(pyomo_model.dXdt_disc_eq)
        #     pyomo_model.del_component(pyomo_model.dXdt_disc_eq_index)
        #     pyomo_model.pprint(filename="femodeljumptest.txt")
        #
        # else:
            ## Model
    ...
    # Sets
        ...
        # m_times1 =sorted(list_times1 & list_feedtimes)
        # list_times=self._meas_times.union(self._feed_times)#combine the two
        # m_times = sorted(m_times1)#sorted(list_times)
        # #print(m_times)
        # m_lambdas = list()
        ...
        if self._absorption_data is not None:
            if not self._meas_times:
                raise RuntimeError('Need to add measurement times')
            # list_times = list(self._meas_times.union(self._feed_times)) #added .union()
        ...
        if self._is_D_deriv == True:# or self._feedadded==True: #or added for feeds
            s_bounds = (None, None)
        else:
            s_bounds = (0.0, None)
        ...
        #pyomo_model.del_component(pyomo_model.D_index)#due to data-matrix modification
        # spectral data
        if self._spectral_data is not None:
            s_data_dict = dict()
            #print('#######################################')
            #pyomo_model.meas_times.pprint()
            for t in pyomo_model.meas_times:
                for l in pyomo_model.meas_lambdas:
                    #if t not in self._feed_times:#For inclusion of discrete feeds CS
                        #print(self._spectral_data[l][t])
                        #print(type(self._spectral_data))
                        #print (type(s_data_dict))
                    # if self._spectral_data[l][t]==0.0:
                    #     s_data_dict[t, l] = 0.0
                    # else:
                    s_data_dict[t, l] = float(self._spectral_data[l][t])
                    #else:
                     #   s_data_dict[t, l] =  0.0 #change for inclusion of jumps for estimation
                    #print(s_data_dict[t,l])
        ...
        self._validate_data(pyomo_model, start_time, end_time)#,modeljump=None)#added 3rd argument
    ...
    @property
    def feed_times(self):
        return self._feed_times #added for feeding points
    ...
    @feed_times.setter
    def feed_times(self):
        raise RuntimeError('Not supported') #added for feeding points
    ...
    def set_non_absorbing_species(self, model, non_abs_list, check=True):
        ...
        #feedtimes = getattr(model, 'feed_times')
